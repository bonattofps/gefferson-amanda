...
const MEDIA_VERSION = "amanda-green-v2";
...
function getWeekAssetPath(folder, week, extension) {
  return `assets/${folder}/semana-${String(week).padStart(2, "0")}.${extension}?v=${MEDIA_VERSION}`;
}
        ...
        <div class="timeline-media" aria-label="Espaços para mídia da semana ${week}">
          <div class="week-media-card">
            <div class="media-frame">
              <span class="media-fallback">Semana ${week}<br>${growth.comparison}</span>
              <img src="${getWeekAssetPath("images", week, "png")}" alt="Imagem ilustrativa do bebê na semana ${week}, tamanho de ${growth.comparison}" loading="${week <= 9 ? "eager" : "lazy"}" decoding="async">
            </div>
            <div>
          ...
          <div class="week-media-card">
            <video class="timeline-video" controls muted loop playsinline preload="${week <= 9 ? "metadata" : "none"}" poster="${getWeekAssetPath("images", week, "png")}" aria-label="Vídeo animado do bebê dentro da barriga da mamãe na semana ${week}">
...
function setupTimelineImages() {
  document.querySelectorAll(".media-frame img").forEach((image) => {
    const markLoaded = () => image.closest(".media-frame")?.classList.add("is-loaded");
    const markFailed = () => image.closest(".media-frame")?.classList.add("has-error");
